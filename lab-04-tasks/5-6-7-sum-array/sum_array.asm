...
add_byte_array_element2:
    ...
    PRINT_STRING "word_array sum is"
    PRINT_UDEC 4, eax
    NEWLINE
...
add_byte_array_element3:
    mov edx, dword [dword_array + ecx * 4 - 4]
    add eax, edx
    loop add_byte_array_element3 ; Decrement ecx, if not zero, add another element.

    PRINT_STRING "dword_array sum is"
    PRINT_UDEC 4, eax
    NEWLINE
...
add_byte_array_element4:
    mov edx, dword [dword_array2 + ecx * 4 - 4]
    mov eax, edx
    mul edx
    add ebx, eax
    loop add_byte_array_element4 ; Decrement ecx, if not zero, add another element.

    PRINT_STRING "Suma patratelor din vectorul dword_array2 este "
    PRINT_UDEC 4, ebx
    NEWLINE
